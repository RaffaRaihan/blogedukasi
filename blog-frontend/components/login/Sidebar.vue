      ...
      <div class="mb-4">
        ...
        <ul class="list-group">
          <li v-for="article in popularArticles" :key="article.id" class="list-group-item">
            ...
            <NuxtLink :to="`/user/articles/${article.ID}`" class="btn btn-success btn-sm mt-2">Lihat</NuxtLink>
          </li>
        </ul>
      </div>
      <div>
        ...
        <ul v-for="category in category" :key="category.id" class="list-group">
          <li class="list-group-item">
            <NuxtLink class="text-muted" to="/user/dashboard">#{{ category.name }}</NuxtLink>
          </li>
        </ul>
      </div>
    </div>
